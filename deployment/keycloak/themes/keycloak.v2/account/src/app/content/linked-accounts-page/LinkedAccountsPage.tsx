...
import {BitbucketIcon, CubeIcon, GitlabIcon, LinkIcon, PaypalIcon, UnlinkIcon} from '@patternfly/react-icons';
...
class LinkedAccountsPage extends React.Component<LinkedAccountsPageProps, LinkedAccountsPageState> {
    ...
    public render(): React.ReactNode {
        ...
        return (
                <ContentPage title={Msg.localize('linkedAccountsTitle')}
        ...
        return (
                <> {
                    ...
                    accounts.map((account: LinkedAccount) => (
                            <DataListItem id={`${account.providerAlias}-idp`} key={account.providerName}
                                    ...
                                    <DataListItemCells
                                            dataListCells={[
                                                <DataListCell key='idp'>
                                                    <Split>
                                                        <SplitItem
                                                                className="pf-u-mr-sm">{this.findIcon(account)}</SplitItem>
                                                        <SplitItem className="pf-u-my-xs" isFilled><span
                                                                id={`${account.providerAlias}-idp-name`}>{account.displayName}</span></SplitItem>
                                                    </Split>
                                                </DataListCell>,
                                                <DataListCell key='label'>
                                                    <Split>
                                                        <SplitItem className="pf-u-my-xs" isFilled><span
                                                                id={`${account.providerAlias}-idp-label`}>{this.label(account)}</span></SplitItem>
                                                    </Split>
                                                </DataListCell>,
                                                <DataListCell key='username' width={5}>
                                                    <Split>
                                                        <SplitItem className="pf-u-my-xs" isFilled><span
                                                                id={`${account.providerAlias}-idp-username`}>{account.linkedUsername}</span></SplitItem>
                                                    </Split>
                                                </DataListCell>,
                                            ]}/>
                                    <DataListAction aria-labelledby={Msg.localize('link')}
                                                    aria-label={Msg.localize('unLink')} id='setPasswordAction'>
                                        {isLinked && <Button id={`${account.providerAlias}-idp-unlink`} variant='link'
                                                             onClick={() => this.unLinkAccount(account)}><UnlinkIcon
                                                size='sm'/> <Msg msgKey='unLink'/></Button>}
                                        {!isLinked && <Button id={`${account.providerAlias}-idp-link`} variant='link'
                                                              onClick={() => this.linkAccount(account)}><LinkIcon
                                                size='sm'/> <Msg msgKey='link'/></Button>}
                                    </DataListAction>
                                </DataListItemRow>
                            </DataListItem>
                    ))
