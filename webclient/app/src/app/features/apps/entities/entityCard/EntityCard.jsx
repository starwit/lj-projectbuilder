...
function EntityCard(props) {
            ...
            <Table size={"small"}>
                <TableHead>
                    <TableRow className={entityCardStyles.tableRow}>
                        <TableCell>{t("field.fieldName")}</TableCell>
                        <TableCell>{t("field.fieldType")}</TableCell>
                    </TableRow>
                </TableHead>
                <TableBody>{renderAttributes(entity)}</TableBody>
            </Table>
        );
    }
    ...
    function prepareDelete() {
        handleDelete(entity.id).then(setIsDeleting(false)).catch(setIsDeleting(false));
    }
    ...
    function renderDeleteWrapper() {
        if (handleDelete && editable) {
            return (
                <Grid item sm={2}>
                    <IconButton onClick={prepareDelete} disabled={isDeleting}>
