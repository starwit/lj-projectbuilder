...
public class GeneratorService {
    ...
    public void generate(Long appId) throws NotificationException {
        ...
        for (TemplateFile templateFile : templateFiles) {
            if (templateFile.isAppend()) {
                ...
                templateFile.getFileName().contains("${domain") || templateFile.getFileName().contains("${entity")
            ) {
    ...
    protected String generatePathWithFreemarker(Map<String, Object> data, String path) throws NotificationException {
        try {
            @SuppressWarnings("deprecation")
            Template templateFileTargetPath = new Template("templatePath", new StringReader(path), new Configuration());
    ...
    protected void generateFileWithOverride(Map<String, Object> data, TemplateFile templateFile)
        ...
        try {
            String targetFileUrl =
                templateFile.getConcreteTargetPath() + generatePathWithFreemarker(data, templateFile.getFileName());
            ...
            throw new NotificationException(
                ...
                "Template with path " + templateUrl + "not found",
                templateUrl
            );
        } catch (FileNotFoundException e) {
            throw new NotificationException(
                "error.generation.filenotfound",
                e.getMessage(),
    ...
    protected void generateAdditionalContent(Map<String, Object> data, TemplateFile templateFile)
        ...
        try {
            addLinesToFile(
                templateFile.getConcreteTargetPath() + Constants.FILE_SEP + templateFile.getFileName(),
                getTemplate(templateFile.getConcreteTemplatePath()),
                data
            );
        } catch (IOException | TemplateException e) {
