...
<div id="bodyBox">
	<div class="contain">
		...
		<fieldset>
			...
			<label for="${attribute.name?lower_case}">{{'${domain.name?lower_case}.${attribute.name}' | translate}}:</label>
			<input name="${attribute.name?lower_case}" id="${attribute.name?lower_case}" type="text" ng-model="ctrl.${domain.name?lower_case}.${attribute.name?uncap_first}" 
			...
			<label for="${attribute.name?lower_case}">{{'${domain.name?lower_case}.${attribute.name}' | translate}}:</label>
			<input name="${attribute.name?lower_case}" id="${attribute.name?lower_case}" type="number" ng-model="ctrl.${domain.name?lower_case}.${attribute.name?uncap_first}" numberinput='' ng-pattern="/^[0-9]+$/"	
			...
			<label for="${attribute.name?lower_case}">{{'${domain.name?lower_case}.${attribute.name}' | translate}}:</label>
			<input name="${attribute.name?lower_case}" id="${attribute.name?lower_case}" type="text" placeholder="_,_" ng-model="ctrl.${domain.name?lower_case}.${attribute.name?uncap_first}" numberinput='' ng-pattern="/^\d{1,4}(\.\d{0,4})?$/"	
				<#if attribute.min??> ng-minlength="${attribute.min}"</#if>
				<#if attribute.max??> ng-maxlength="${attribute.max}"</#if>
				<#if !attribute.nullable> required</#if>			
			/>
			...
			<input name="${attribute.name?lower_case}" id="${attribute.name?lower_case}" 
				type="text" moment-picker="ctrl.${domain.name?lower_case}${attribute.name?uncap_first}formated" ng-model="ctrl.${domain.name?lower_case}.${attribute.name?uncap_first}" ng-value="ctrl.${domain.name?lower_case}.${attribute.name?uncap_first} | formatLocalDate"
			<#if !attribute.nullable> required</#if>	
			/>
			</#if>
			...
			<label for="${attribute.name?lower_case}">{{'${domain.name?lower_case}.${attribute.name}' | translate}}:</label>
			<input name="${attribute.name?lower_case}" id="${attribute.name?lower_case}" 
				type="text" moment-picker="ctrl.${domain.name?lower_case}${attribute.name?uncap_first}formated"
				ng-model="ctrl.${domain.name?lower_case}.${attribute.name?uncap_first}" ng-value="ctrl.${domain.name?lower_case}.${attribute.name?uncap_first} | formatLocalTime"
		...
		<div class="submitArea">
			<button class="buttonRounded" ng-click="ctrl.goto${domain.name}.all();"><img src="res/img/icons/back.png" border="0" /><span class="hidden">{{'button.back' | translate}}</span></button>
